...
describe('gstack-slug', () => {
  ...
  test('no tracked templates or bin scripts use source process substitution for gstack-slug', () => {
    const result = Bun.spawnSync(
      ['git', 'grep', '-n', 'source <(.*gstack-slug', '--', '*.tmpl', 'bin/gstack-review-*'],
      { cwd: ROOT, stdout: 'pipe', stderr: 'pipe' }
    );
    // git grep returns exit code 1 when no matches found — that's what we want
