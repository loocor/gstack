...
2. **If no override exists,** use AskUserQuestion:
   ...
   - For Design Review: run `source <(~/.gstack/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**If "Add as P0 TODO":**
- If `TODOS.md` exists, add the entry following the format in `review/TODOS-format.md` (or `~/.gstack/review/TODOS-format.md`).
...
1. Read `~/.gstack/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
source <(~/.gstack/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `~/.gstack/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
```bash
~/.gstack/bin/gstack-review-log '{"skill":"design-review-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"commit":"COMMIT"}'
```
...
Read `~/.gstack/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
# Respect old opt-out
OLD_CFG=$(~/.gstack/bin/gstack-config get codex_reviews 2>/dev/null || true)
...
```bash
~/.gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"medium","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
```bash
~/.gstack/bin/gstack-review-log '{"skill":"adversarial-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","tier":"large","gate":"GATE","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
Read `~/.gstack/review/TODOS-format.md` for the canonical format reference.
